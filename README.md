...
| **消息记录** | 侧边栏「消息记录」。真收真发的聊天页：左侧群聊 / 私聊列表（搜索、未读、按最近活跃排序），中间真实消息流（头像、昵称、时间、引用、合并转发可展开），底部「普通 / 发送 Raw」两档输入，支持引用回复与撤回。详见下方「消息记录说明」 |
...
- **发送图片目前不可用**：图片是 base64 塞进 JSON 请求体的，会撞上后端的 body 大小上限（浏览器里表现为 `Request failed with status code 413`）。收图正常显示，发图待改成 multipart 上传后再放开
