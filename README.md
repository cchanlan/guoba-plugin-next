...
| 功能 | 说明 |
| --- | --- |
| **运行日志** | 侧边栏「运行日志」。实时查看 Bot 日志，支持按级别筛选、关键词搜索、清空。增量拉取，不重复传已看过的行。「发给主人」把当前显示的日志渲染成图片私聊发给主人；聊天里发 `#锅巴日志`（`#锅巴日志30` 指定条数、`#锅巴错误日志` 只看报错）也能拿到同一套样式的图。「文件清理」可设定时清理 Yunzai `logs` 目录（默认每月 1 号凌晨清掉 30 天前的归档，`error.log` 超过 5MB 截断），也能立刻清一次 |
...
| **文件管理** | 侧边栏「文件管理」。浏览 / 编辑 **Yunzai 根目录**下的文件：目录列表 + 面包屑，文本文件在页面里改，支持上传 / 下载、新建 / 重命名 / 删除；文件夹可以算占用、可以打包成 zip 下载（边打边传，几万文件也不占磁盘）。访问限定根目录内，二进制 / 超大文件只能下载。详见下方「文件管理说明」 |
| **插件更新** | 插件管理页。卡片上显示每个插件的分支、当前提交、上次检查时间；「检查更新」逐个 `git fetch` 后标出落后几个提交，可单个更新或一次更新全部。遇到本地改动默认跳过，也能选「暂存改动」或「丢弃改动强制更新」；`package.json` 变了自动装依赖，更新完可选是否重启，出问题能一键回滚到更新前的提交。详见下方「插件更新说明」 |
...
- **文件夹的大小要点「计算」才算**：列目录时顺手统计的话，`node_modules` 一层就是十万个 inode，进目录就卡住。撞上内置上限（50 万条目 / 20 秒）会显示成「≥ 多少」，表示只数到这儿
- **下载文件夹**会打包成 zip：边遍历边生成边传，内存恒定、不落临时文件，所以没有进度百分比（浏览器显示未知大小）。实测 879MB / 7.6 万文件的 `node_modules` 打成 246MB 包耗时约 50 秒，内存峰值 146MB。中途关掉页面就是取消，服务端会立刻收工

### 六、插件更新说明

插件管理页在原来的装 / 卸之外补上了更新。检查和更新是**两步**，因为检查要联网：

- **平时只读本地 git 状态**（分支、当前提交、有没有本地改动），随页面一起加载，很快；卡片上的「落后 N 个提交」是**上次检查时**的结果，所以旁边会写「几分钟前检查」
- 筛选器里有**「可更新」**一档，检查完直接切过去看，不用自己去搜插件名
- **「检查更新」**才会逐个 `git fetch`（同时最多 3 个，再多容易被 GitHub 限流），完了标出每个插件落后几个提交、新提交都是什么
- **更新用 `merge --ff-only`**，不是 `git pull`：pull 在历史分叉时会尝试 merge、弄出 merge commit 甚至冲突半途而废，插件目录就成了要人工收拾的烂摊子。ff-only 要么干净快进，要么原地不动报错
- **遇到本地改动默认跳过**。自己改过插件代码的话，另外两个模式是：「暂存改动」（`git stash -u`，更新完放回；跟上游冲突时改动**留在 stash 里不丢**，页面会提示）和「丢弃改动强制更新」（`reset --hard`，本地改动和未推送的提交都会没）
- 未跟踪文件（自己塞的资源、日志）**不算「有改动」**，不会因为它们把插件判成不能更新
- **「有本地改动」很常见**：不少插件把配置或缓存写在自己仓库里的跟踪文件上（实测 26 个插件里有 12 个是这样，最多的一个有 56 个文件被改过），它们在默认模式下会被跳过。卡片上的「本地有改动」标签悬浮能看到具体文件，确认是插件自己写的就用「暂存改动」模式
- **`package.json` 变了才装依赖**，走 `pnpm --filter ./plugins/<名字> install`，规则和备份还原那边共用一套（含 README 里写的额外依赖）
- **更新完不自动重启**，要不要重启由你勾选 —— 正在聊天的群不该因为面板上点了个更新就断一下。依赖没装上时会跳过自动重启，避免满屏缺模块报错
- **能回滚**：更新成功后卡片上出现「回滚」，`reset` 回更新前那个提交（依赖不动）。这个记录只在内存里，锅巴重启后回滚入口就消失 —— 隔了一次重启再回滚，风险比收益大
- 私有仓库、链接失效、没配上游、HEAD 游离、不是 git 仓库（手动解压装的）都会**说明原因并跳过**，不会卡在那儿等 git 要密码
