...
| 功能 | 说明 |
| --- | --- |
| **运行日志** | 侧边栏「运行日志」。实时查看 Bot 日志，支持按级别筛选、关键词搜索、清空。增量拉取，不重复传已看过的行。「发给主人」把当前显示的日志渲染成图片私聊发给主人；聊天里发 `#锅巴日志`（`#锅巴日志30` 指定条数、`#锅巴错误日志` 只看报错）也能拿到同一套样式的图。「日志清理」可设定时清理 Yunzai `logs` 目录（默认每月 1 号凌晨清掉 30 天前的归档，`error.log` 超过 5MB 截断），也能立刻清一次 |
