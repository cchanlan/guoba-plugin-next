...
- 新增暗色 / 亮色主题，跟随系统或手动切换（登录页也能切，与面板内共用一份状态）
- 图标改用内置的 ant-design 图标集（`web/src/components/icons/offline.ts`），**不依赖 iconify 公共 API**，离线环境也能正常显示
- 移动端适配：手机上消息记录是「列表 / 会话」两屏切换（标题栏带返回键），消息记录 / 运行日志 / 沙盒三页整页不滚、内容容器铺满可视高度并自动贴底，页面顶部的标题与说明文案在窄屏隐藏，高度全留给内容
...
| **沙盒** | 侧边栏「沙盒」。在网页里伪造一条消息喂给插件加载器，把回复截在面板里显示，不发到 QQ。支持群聊 / 私聊切换、场景配置（发送者 QQ、群号、主人 / 群主 / 管理员、是否 @机器人）、插件规则浏览与正则匹配预览、图片收发、引用消息（插件读 `e.getReply()` 能拿到内容），回复里的按钮可以直接点，Markdown、转发消息、语音视频均能渲染。详见下方「沙盒说明」 |
...
| **登录安全** | 账号密码登录 + 新设备首次登录要验证码（机器人私聊发给主人）。验证过的设备会在浏览器里存一份 90 天的长期凭证（每次登录自动换新，后端只存哈希），认设备不认 IP，手机流量 IPv6 漂移也不用反复验证；「账号管理 - 登录安全」里可查看 / 撤销可信设备与可信 IP。忘记密码发 `#锅巴重置密码` 重新初始化 |
...
- **首次安装引导改为文字版**：原版只发一张图片（`resources/images/help.jpg`），图上写的「默认用 localhost 作为访问地址」「发 `#锅巴帮助` 获取修改地址帮助」「有新版本一般会自动更新」都已经和现在的实现对不上，改为文字引导并对齐当前登录流程（`#锅巴登录` 拿地址 → 登录页点「获取登录令牌」→ 登录后设置账号密码）
- **首次安装引导发送时机**：原版在插件加载阶段就发、且先把 `guide` 标记置 false 再发送，那会儿 Bot 账号还没连上，消息发不出去而标记已经关掉，引导就永久丢失了；现在等真实账号 `connect` 后延迟发送（忽略 `stdin` 这类非真实账号，只发给该账号名下的主人），确认发送成功才关标记，失败保留到下次启动重试
...
- **复制**：复制这条消息的纯文本
- **查看原始消息**：弹窗分四档 —— **段数组**（`e.message`）、**上报原文**（适配器收到的 OneBot 上报）、**pb 元素** 与 **pb 原文**（走协议取，路径与过滤规则照 Packet-plugin 的 `#取` 来，看到的跟群里 `#取` 一致）。前两档优先现调 `get_msg`，比内存缓冲里的全；后两档需要装 [Packet-plugin](https://gitee.com/HDTianRu/Packet-plugin) 并让协议端开启 packet 模式（NapCat），缺什么页面就如实说明，不留白
...
在任意一条气泡上右键可以**引用**它再发下一条，插件里 `e.getReply()` 与 `e.source` 都能读到被引用的内容（字段照 icqq 的 `getMsg()` 对齐），取图、复读这类要读引用的插件在沙盒里也能跑。沙盒的历史只存在浏览器 localStorage 里、服务端不留会话，所以引用内容由前端一起带回；只认文本和图片段（按钮、Markdown、转发这些插件读引用时基本用不上），被引用图片无论来自机器人回复、你自己上传还是 http 直链，都会转成 Yunzai 认的形式交给插件。
...
发送 `#锅巴更新` 可手动更新，发送 `#锅巴强制更新` 可强制同步当前远端分支。更新成功后，机器人会用合并转发自动显示本次新增的更新记录。

发送 `#锅巴更新日志` 可查看本 Fork 从建库以来的全部历史更新；日志随仓库保存，浅克隆安装也能完整查看。
